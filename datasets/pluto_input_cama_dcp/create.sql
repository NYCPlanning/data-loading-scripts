-- create table to load csv from the nyc open data portal
DROP TABLE IF EXISTS pluto_input_cama_dcp;
CREATE TABLE pluto_input_cama_dcp (
	BORO text,
	BLOCK text,
	LOT text,
	EASEMENT text,
	PARCELCARD text,
	BLDGNUM text,
	BLDGCLASS text,
	PRIMARYUSECODE text,
	DEVELOPMENTNAME text,
	STREETTYPE text,
	LOTTYPE text,
	RESIDAREA text,
	OFFICEAREA text,
	RETAILAREA text,
	GARAGEAREA text,
	STORAGEAREA text,
	FACTORYAREA text,
	OTHERAREA text,
	GROSSAREA text,
	OWNERAREA text,
	GROSSVOLUME text,
	COMMERCIALAREA text,
	PROXCODE text,
	BSMNT_TYPE text,
	BSMNTGRADIENT text,
	BSMNTAREA text,
	FIRSTFLOORAREA text,
	SECONDFLOORAREA text,
	UPPERFLOORAREA text,
	PARTRESFLOORAREA text,
	UNFINISHEDFLOORAREA text,
	FINISHEDFLOORAREA text,
	NONRESIDFLOORAREA text,
	RESIDCONSTRTYPE text,
	RESIDCTOVERFLOW text,
	COMMERCIALCONSTRTYPE text,
	COMMERCIALCTOVERFLOW text,
	CONDOMAINCONSTRTYPE text,
	CONDOMAINCTOVERFLOW text,
	CONDOUNITSCONSTRTYPE text,
	CONDOUNITSCTOVERFLOW text
  )